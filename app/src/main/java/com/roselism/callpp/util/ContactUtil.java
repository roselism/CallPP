...
public class ContactUtil  {
    ...
    private static List<ContactInfo> getContacts(Context context, Uri uri) {
        ...
        Cursor cursor = resolver.query(uri, CONTACT_PROJECTION, null, null, null, null);
        List<ContactInfo> list = new ArrayList<>();
        if (cursor != null) {
            while (cursor.moveToNext()) {
                //取出数据
                String name = cursor.getString(cursor.getColumnIndex(CONTACT_PROJECTION[0]));
                String number = cursor.getString(cursor.getColumnIndex(CONTACT_PROJECTION[1]));
                if (name == null)
                    name = number;
                ...
                //把联系人信息封装到集合中
                ContactInfo contactInfo = new ContactInfo(name, number, timesContacted, lastTime, hasPhoneNumber, contactID, contactPhoto, lookupKey);
