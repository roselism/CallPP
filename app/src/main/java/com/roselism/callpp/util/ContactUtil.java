...
public class ContactUtil {
	...
	private static List<ContactInfo> getContacts(Context context, Uri uri) {
		...
		Cursor cursor = resolver.query(uri, CONTACT_PROJECTION, null, null,
				"name asc", null);
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				...
				String name = cursor.getString(cursor
						.getColumnIndex(CONTACT_PROJECTION[0]));
				String number = cursor.getString(cursor
						.getColumnIndex(CONTACT_PROJECTION[1]));
				if (name == null)
					name = number;
				int timesContacted = cursor.getInt(cursor
						.getColumnIndex(CONTACT_PROJECTION[2]));
				long lastTime = cursor.getLong(cursor
						.getColumnIndex(CONTACT_PROJECTION[3]));
				int hasPhoneNumber = cursor.getInt(cursor
						.getColumnIndex(CONTACT_PROJECTION[4]));
				long contactID = cursor.getLong(cursor
						.getColumnIndex(CONTACT_PROJECTION[5]));
				long photoID = cursor.getLong(cursor
						.getColumnIndex(CONTACT_PROJECTION[6]));
				String lookupKey = cursor.getString(cursor
						.getColumnIndex(CONTACT_PROJECTION[7]));
				// //得到联系人头像Bitamp
				Bitmap contactPhoto = null;
				// photoid 大于0 表示联系人有头像 如果没有给此人设置头像则给他一个默认的
				if (photoID > 0) {
					Uri phontoUri = ContentUris.withAppendedId(
							ContactsContract.Contacts.CONTENT_URI, contactID);
					InputStream input = ContactsContract.Contacts
							.openContactPhotoInputStream(resolver, phontoUri);
					contactPhoto = BitmapFactory.decodeStream(input);
				} else {
					contactPhoto = BitmapFactory.decodeResource(
							UIUtils.getResources(), R.mipmap.ic_contact);
				}
				// 把联系人信息封装到集合中
